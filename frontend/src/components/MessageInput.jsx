...
const MessageInput = forwardRef(({
  ...
  const toggleListening = useCallback(() => {
    ...
    recognition.lang = user?.profile?.preferences?.voice_language || navigator.language || 'en-US';
  ...
  const modelOptions = useMemo(() => {
    ...
    for (const provider of providers.all) {
      if (connectedProviders.length > 0 && !connectedProviders.includes(provider.id)) {
        continue;
      }
  ...
  const handleSubmit = (e) => {
    ...
    const allUploaded = attachedFiles.every((f) => f.uploaded || !f.uploading);
  ...
  const canSend = (hasContent || hasUploadedFiles) && !disabled && !isUploading;

  return (
    <Box sx={{ px: 2.5, pb: 2.5, pt: 1.5, backgroundColor: 'var(--bg-primary)' }}>
      <Sheet
        variant="outlined"
            ...
            {attachedFiles.map((fileEntry, index) => (
              ...
              >
                ...
                {fileEntry.preview ? (
                  ...
                  >
                    <InsertDriveFileOutlined sx={{ fontSize: 24, color: 'neutral.500' }} />
                  </Box>
                )}
        ...
        {isDragOver && (
          <Box
            sx={{
              ...
              alignItems: 'center',
              justifyContent: 'center',
              backgroundColor: 'rgba(var(--joy-palette-primary-mainChannel) / 0.08)',
              zIndex: 10,
              pointerEvents: 'none',
              borderRadius: 'lg',
            }}
          >
            <Typography level="body-sm" sx={{ color: 'primary.500', fontWeight: 'md' }}>
              ...
              >
                {agents.map((agent) => (
                  <Button
                    ...
                    color={selectedAgent === agent.name ? 'primary' : 'neutral'}
            ...
            >
              {modelOptions.map((opt) => (
                <Option
                  ...
                  label={opt.modelName}
                >
                  <ListItemDecorator sx={{ fontSize: '10px', color: 'text.tertiary', minWidth: 0, mr: 1 }}>
                    {opt.providerName}
                  </ListItemDecorator>
                  {opt.modelName}
                </Option>
