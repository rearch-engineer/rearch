...
export default function AuthCallbackPage() {
  ...
  useEffect(() => {
    ...
    const processCallback = async () => {
      ...
      if (!stateToken) {
        toast.error('Missing state token. Please try logging in again.');
        navigate('/login', { replace: true });
        return;
      }
    ...
    return (
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '100vh', bgcolor: 'background.level1' }}>
        <Card variant="outlined" sx={{ width: 450, p: 4, textAlign: 'center' }}>
          <Typography level="h4" sx={{ mb: 2 }}>Account Pending Approval</Typography>
  ...
  return (
    <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '100vh', bgcolor: 'background.level1' }}>
      <Card variant="outlined" sx={{ width: 300, p: 4, textAlign: 'center' }}>
        ...
        <Typography level="body-md">Completing sign-in...</Typography>
      </Card>
    </Box>
  );
}
