...
export default function StartPage() {
  ...
  useEffect(() => {
    ...
    if (!repoName) {
      setError("No repository name specified in the URL. Expected /start#<repository-name>");
      return;
    }
    ...
    const createAndRedirect = async () => {
      try {
        ...
        if (status === 404 && message === "repository-not-found") {
          setError(
            `Repository "${decodeURIComponent(repoName)}" was not found or is not enabled for ReArch.`
          );
        } else {
