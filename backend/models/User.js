...
// Never expose password_hash or raw integration tokens in JSON
userSchema.methods.toSafeJSON = function () {
  const obj = this.toObject();
  delete obj.auth.password_hash;

  // Never leak the encrypted GitHub Copilot token; expose only a derived
  // `connected` boolean and the `connected_at` timestamp.
  if (!obj.tools) obj.tools = {};
  obj.tools.github_copilot = {
    connected: !!obj.tools?.github_copilot?.token?.ciphertext,
    connected_at: obj.tools?.github_copilot?.connected_at || null,
  };
